...
type WorldManager struct {
	Players map[uint32]*pb.Player //当前在线的玩家集合
...
func (wm *WorldManager) AddPlayer(player *pb.Player) {
	//将player添加到 世界管理器中
	wm.pLock.Lock()
	wm.Players[player.Id] = player
	wm.pLock.Unlock()
...
//通过玩家ID 获取对应玩家信息
func (wm *WorldManager) GetPlayerByPID(pID uint32) *pb.Player {
	wm.pLock.RLock()
...
//获取所有玩家的信息
func (wm *WorldManager) GetAllPlayers() []*pb.Player {
	wm.pLock.RLock()
	defer wm.pLock.RUnlock()
